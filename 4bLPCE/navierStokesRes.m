...
function [Sol, Force] = navierStokesRes(solver, fluid, pmc, Sol, cnn, crd, ...
...
Sol.u(oisd,1,1) = (velSAlpha(oisd,1,1) - Sol.uPrev(oisd,1,1))./pmc.alpha + Sol.uPrev(oisd,1,1) ;
Sol.u(oisd,2,1) = (velSAlpha(oisd,2,1) - Sol.uPrev(oisd,2,1))./pmc.alpha + Sol.uPrev(oisd,2,1) ;
Sol.uAlpha = Sol.uPrev + pmc.alpha.*(Sol.u - Sol.uPrev) ;

Sol.aleVel(oisd,1,1) = (velSAlpha(oisd,1,1) - Sol.aleVelPrev(oisd,1,1))./pmc.alpha + Sol.aleVelPrev(oisd,1,1) ;
Sol.aleVel(oisd,2,1) = (velSAlpha(oisd,2,1) - Sol.aleVelPrev(oisd,2,1))./pmc.alpha + Sol.aleVelPrev(oisd,2,1) ;
...
[RHS] = GalerkinTermsRHS(Sol, fluid, pmc, solver, iif, jjf, xxf, yyf, ux, uy, ...
                           gW, N, Nx, Ny, nElem, nQuad, nen, ndof, alevelx, alevely);
                                
% Form element matrix and assemble Petrov-Galerkin terms
[RHS] = PetrovGalerkinTerms1RHS(Sol, fluid, pmc, solver, iif, jjf, ...
                                  xxf, yyf, ux, uy, gW, N, Nx, Ny, ...
                                  nElem, nQuad, nen, ndof, elemType, ...
                                  RHS, alevelx, alevely);
[RHS] = PetrovGalerkinTerms2RHS(Sol, fluid, pmc, solver, iif, jjf, ...
                                  xxf, yyf, ux, uy, gW, N, Nx, Ny, ...
                                  nElem, nQuad, nen, ndof, elemType, ...
                                  RHS, alevelx, alevely);
[RHS] = PetrovGalerkinTerms3RHS(Sol, fluid, pmc, solver, iif, jjf, ...
                                  xxf, yyf, ux, uy, gW, N, Nx, Ny, ...
                                  nElem, nQuad, nen, ndof, elemType, ...
                                  RHS, alevelx, alevely);
        

% Obtain integrated forces using residuals on the fsi nodes
